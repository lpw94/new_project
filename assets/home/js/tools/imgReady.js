...
// 重置窗口大小(全部显示图片)
function resize_full_img(w_w, w_h, i_w, i_h) {
    var ratio = i_w / i_h;
    var width = w_w != 0 ? w_w : 200;
    var height = w_h != 0 ? w_h : 200;
    var frameRatio = w_w / w_h;//外框大小比例
    ...
    if (ratio >= frameRatio) {
        //宽一点 最大边 取宽
        newImgHeight = height / ratio;
        newImgWidth = width;
        offestY = (height - newImgHeight) / 2;
        offestX = 0;
    } else {
        //窄一点 最小边宽 取宽
        newImgWidth = width * ratio;
        newImgHeight = height;
        offestX = (width - newImgWidth) / 2;
        offestY = 0;
    }
