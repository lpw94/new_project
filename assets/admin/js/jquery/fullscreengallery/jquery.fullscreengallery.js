    ...
    $.fn.fullScreenGallery = function (settings) {
        ...
        $(this).each(function (i) {
            if ($(this).attr('src')) {
                ...
                settings.titleArray.push($(this).attr('title'))
            }
        });
        ...
        function _set_interface() {
            ...
            $('#picView-imgViewBox').css({
                width: (_getWindowSize()[0] - 0) + 'px',
                height: _getWindowSize()[1],
                lineHeight: _getWindowSize()[1] + 'px'
            });
            $('#picView-overlay').show().css({
                width: (_getWindowSize()[0] - 0),
                height: _getWindowSize()[1]
            });
            $('#overlay-panel').css({
                height: _getWindowSize()[1]
            ...
            //下一张
            $('#picView-next').on('click', function (e) {
            ...
            //上一张
            $('#picView-prev').on('click', function (e) {
                settings.activeImage -= 1;
            ...
            //关闭
            $('#picView-close').on('click', function (e) {
                _finish()
                e.stopImmediatePropagation();
            });
        ...
        function _resizeImg(jqObj) {
            ...
            //如果图片高或者宽超过屏幕
            if (jqObj.attr('ow') > (arrPageSizes[0] - 0) || jqObj.attr('oh') > (arrPageSizes[1] - 72)) {
                var s1 = (arrPageSizes[0] - 0) / (arrPageSizes[1] - 72);
                var s2 = jqObj.attr('ow') / jqObj.attr('oh');

                if (s2 > s1) {
                    //横图 缩小
                    var smallHeight = parseInt((arrPageSizes[0] - 0) * jqObj.attr('oh') / jqObj.attr('ow'));
                    jqObj.animate({
                        'width': arrPageSizes[0] - 0,
                        'height': smallHeight,
                        'left': 0,
                        'top': parseInt(((arrPageSizes[1] - 72) - smallHeight) / 2)
                    });
                    ...
                    jqObj.animate({
                        'width': smallWidth,
                        'height': (arrPageSizes[1] - 72),
                        'left': parseInt((arrPageSizes[0] - 0 - smallWidth) / 2),
                        'top': 0
                    }, function () {
                        jqObj.attr('zoom', 'small');
                        $('#zoomOut').show();
                ...
                function _zoomOut(jqObj) {
                    ...
                    jqObj.animate({
                        'width': jqObj.attr('ow'),
                        'height': jqObj.attr('oh'),
                        'left': parseInt((arrPageSizes[0] - 0 - jqObj.attr('ow')) / 2),
                        'top': parseInt(((arrPageSizes[1] - 72) - jqObj.attr('oh')) / 2)
                    }, function () {
                        jqObj.attr('zoom', 'big');
                        $('#zoomOut').hide();
                        $('#zoomIn').show();
                ...
                jqObj.animate({
                    'width': 'ow',
                    'height': 'oh',
                    'top': ((arrPageSizes[1] - 72) - oh) / 2,
                    'left': (arrPageSizes[0] - 0 - ow) / 2
                });
            }
        }
        ...
        function _createImg(n) {
            ...
            // 标题显示
            $('#picView-overlay #overlay-header').html(settings.titleArray[settings.activeImage]);
        }
